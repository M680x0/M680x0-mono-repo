...
SDValue M68kTargetLowering::EmitTailCallLoadRetAddr(
...
M68kTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  //           DAG.getNode(M68kISD::GlobalBaseReg, SDLoc(),
  //           getPointerTy(DAG.getDataLayout()))));
  //   } else {
  //     // ??? WUT, debug this
...
SDValue M68kTargetLowering::EmitCmp(SDValue Op0, SDValue Op1,
                                      unsigned M68kCC, const SDLoc &DL,
                                      SelectionDAG &DAG) const {
...
SDValue M68kTargetLowering::LowerSETCCCARRY(SDValue Op,
  ...
  M68k::CondCode CC =
      TranslateIntegerM68kCC(cast<CondCodeSDNode>(Cond)->get());
...
static bool isM68kLogicalCmp(SDValue Op) {
  ...
  if (Opc == M68kISD::CMP)
    return true;
  if (Op.getResNo() == 1 &&
      (Opc == M68kISD::ADD || Opc == M68kISD::SUB ||
       Opc == M68kISD::ADDX || Opc == M68kISD::SUBX ||
       Opc == M68kISD::SMUL || Opc == M68kISD::UMUL ||
  ...
  if (CondOpcode == ISD::UADDO || CondOpcode == ISD::SADDO ||
    ...
    if (Cond.hasOneUse() && isAndOrOfSetCCs(Cond, CondOpc)) {
      ...
      if (CondOpc == ISD::OR) {
        ...
        if (Cmp == Cond.getOperand(1).getOperand(1) &&
            isM68kLogicalCmp(Cmp) && Op.getNode()->hasOneUse()) {
          M68k::CondCode CCode =
...
SDValue M68kTargetLowering::LowerConstantPool(SDValue Op,
  ...
  if (M68kII::isGlobalRelativeToPICBase(OpFlag)) {
    Result = DAG.getNode(ISD::ADD, DL, PtrVT,
                         DAG.getNode(M68kISD::GlobalBaseReg, SDLoc(), PtrVT),
                         Result);
  }
  ...
  if (M68kII::isGlobalRelativeToPICBase(OpFlags)) {
    Result =
        DAG.getNode(ISD::ADD, DL, PtrVT,
                    DAG.getNode(M68kISD::GlobalBaseReg, DL, PtrVT), Result);
  }
...
M68kTargetLowering::getPICJumpTableRelocBase(SDValue Table,
  ...
  if (getJumpTableEncoding() == MachineJumpTableInfo::EK_Custom32)
    return DAG.getNode(M68kISD::GlobalBaseReg, SDLoc(),
                       getPointerTy(DAG.getDataLayout()));
...
M68kTargetLowering::EmitLoweredSelect(MachineInstr &MI,
  ...
  for (MachineBasicBlock::iterator MIIt = MIItBegin; MIIt != MIItEnd; ++MIIt) {
    ...
    MIB = BuildMI(*sinkMBB, SinkInsertionPoint, DL, TII->get(M68k::PHI),
                  DestReg)
              .addReg(Op1Reg)
              .addMBB(copy0MBB)
              .addReg(Op2Reg)
...
SDValue M68kTargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
...
const char *M68kTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case M68kISD::GlobalBaseReg:
    return "M68kISD::GlobalBaseReg";
