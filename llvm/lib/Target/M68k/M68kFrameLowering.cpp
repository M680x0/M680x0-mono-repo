...
M68kFrameLowering::M68kFrameLowering(const M68kSubtarget &STI,
                                         Align Alignment)
...
bool M68kFrameLowering::hasReservedCallFrame(
...
int M68kFrameLowering::getFrameIndexReference(const MachineFunction &MF,
                                                int FI,
  ...
  const M68kMachineFunctionInfo *MMFI =
      MF.getInfo<M68kMachineFunctionInfo>();
...
static unsigned getSUBriOpcode(int64_t Imm) { return M68k::SUB32ri; }

static unsigned getADDriOpcode(int64_t Imm) { return M68k::ADD32ri; }

static unsigned getSUBrrOpcode() { return M68k::SUB32rr; }

static unsigned getADDrrOpcode() { return M68k::ADD32rr; }

static unsigned getANDriOpcode(int64_t Imm) { return M68k::AND32di; }

static unsigned getLEArOpcode() { return M68k::LEA32p; }
...
bool M68kFrameLowering::assignCalleeSavedSpillSlots(
  ...
  int SpillSlotOffset =
      getOffsetOfLocalArea() + M68kFI->getTCReturnAddrDelta();
...
bool M68kFrameLowering::spillCalleeSavedRegisters(
  ...
  auto I =
      addFrameReference(BuildMI(MBB, MI, DL, TII.get(M68k::MOVM32pm)), FI)
          .addImm(Mask)
          .setMIFlag(MachineInstr::FrameSetup);
