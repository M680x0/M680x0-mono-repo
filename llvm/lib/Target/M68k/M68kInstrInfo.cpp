...
bool M68kInstrInfo::isRegisterOperandPCRel(const MachineOperand &MO) const {
  assert(MO.isReg());
...
void M68kInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (M68k::DR8RegClass.contains(SrcReg)) {
    ...
    if (M68k::XR32RegClass.contains(DstReg)) {
      Opc = M68k::MOVXd32d16;
    }
  }
...
unsigned getLoadStoreRegOpcode(unsigned Reg, const TargetRegisterClass *RC,
  ...
  case 8:
    ...
    } else if (M68k::CCRCRegClass.hasSubClassEq(RC)) {
...
void M68kInstrInfo::loadRegFromStackSlot(
