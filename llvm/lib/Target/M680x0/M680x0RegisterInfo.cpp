...
static cl::opt<bool> EnableBasePointer(
    "m680x0-use-base-pointer", cl::Hidden, cl::init(true),
...
M680x0RegisterInfo::getRegsForTailCall(const MachineFunction &MF) const {
  return &M680x0::XR32_TCRegClass;
}

unsigned
M680x0RegisterInfo::getMatchingMegaReg(unsigned Reg,
...
int M680x0RegisterInfo::getRegisterOrder(unsigned Reg,
...
int M680x0RegisterInfo::getSpillRegisterOrder(unsigned Reg) const {
...
BitVector M680x0RegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  setBitVector(M680x0::PC);
  setBitVector(M680x0::SP);
...
void M680x0RegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
...
bool M680x0RegisterInfo::requiresRegisterScavenging(
    const MachineFunction &MF) const {
  return true;
}

bool M680x0RegisterInfo::trackLivenessAfterRegAlloc(
    const MachineFunction &MF) const {
  return true;
}
...
bool M680x0RegisterInfo::hasBasePointer(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
bool M680x0RegisterInfo::canRealignStack(const MachineFunction &MF) const {
...
const TargetRegisterClass *
M680x0RegisterInfo::intRegClass(unsigned size) const {
  // if (isInt<8>(size)) {
